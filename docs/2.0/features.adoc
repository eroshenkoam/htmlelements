...
one "step" method per action as its implementation. That is where all the extension methods from ExtendedWebElement
...
*Waiting for page loading.* There is a special `isAt(Matcher<String> url)` method that waits on the condition for page's
...
To navigate between this two pages you have to wait for the account page to load after clicking on the 'Account' button.
