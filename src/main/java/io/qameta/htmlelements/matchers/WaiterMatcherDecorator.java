...
public class WaiterMatcherDecorator<T> extends TypeSafeMatcher<T> {
    ...
    @Factory
    public static <T> Matcher<T> withWaitFor(final Matcher<? super T> matcher, final Waiter waiter) {
        return new WaiterMatcherDecorator<>(matcher, waiter);
    }

    @Factory
    public static <T> Matcher<T> withWaitFor(final Matcher<? super T> matcher) {
        return new WaiterMatcherDecorator<>(matcher, new TimeoutWaiter(50000));
    }
}